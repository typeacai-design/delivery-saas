          ...
          <div className="grid grid-cols-2 gap-3">
            {produtosSessao.map((produto: any) => (
              <button
                key={produto.id}
                onClick={() => onAbrirModal(produto)}
              className="wd-product-card rounded-2xl overflow-hidden text-left hover:shadow-lg transition-all active:scale-95"
              >
                <div className="wd-product-image aspect-square relative">
                  {produto.imagem_url ? (
                    <img src={produto.imagem_url} alt={produto.nome} className="w-full h-full object-cover" />
                  ) : (
                    <div className="w-full h-full flex items-center justify-center text-5xl">🍔</div>
