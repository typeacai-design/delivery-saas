...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (!file || !TYPES.includes(file.type)) return NextResponse.json({ error: 'Use JPG, PNG ou WebP.' }, { status: 400 })
    if (file.size > 5 * 1024 * 1024) return NextResponse.json({ error: 'Arquivo maior que 5MB.' }, { status: 400 })
    const path = `${tenantId}/complementos/${crypto.randomUUID()}.${extension(file.type)}`
...
export async function DELETE(request: Request) {
  ...
  if (!path?.startsWith(`${tenantId}/complementos/`)) return NextResponse.json({ error: 'Path invalido' }, { status: 400 })
  const { error } = await supabase.storage.from('complementos').remove([path])
